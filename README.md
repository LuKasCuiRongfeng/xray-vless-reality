...
在 Linux 服务器 (Debian/Ubuntu/CentOS 等) 上执行, 把地址中的 USERNAME/REPO 换成你的 GitHub 仓库:
    ...
    curl -fsSL https://raw.githubusercontent.com/USERNAME/REPO/main/install.sh -o install.sh
    sudo bash install.sh install

或一行管道方式 (建议先下载查看内容再执行):

    curl -fsSL https://raw.githubusercontent.com/USERNAME/REPO/main/install.sh | sudo bash install.sh install
